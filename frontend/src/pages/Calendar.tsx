...
const appointmentTypes = {
    consultation: "text-green-600 bg-green-100 dark:text-green-400 dark:bg-green-900",
    massage: "text-blue-600 bg-blue-100 dark:text-blue-400 dark:bg-blue-900",
    followup: "text-purple-600 bg-purple-100 dark:text-purple-400 dark:bg-purple-900",
};
...
const Calendar: React.FC = () => {
    ...
    const [appointments, setAppointments] = useState<Appointment[]>([
        { id: 1, date: today.date(5).format("YYYY-MM-DD"), title: "Client A - 2 PM", type: "massage" },
        { id: 2, date: today.date(10).format("YYYY-MM-DD"), title: "Client B - 4 PM", type: "consultation" },
        { id: 3, date: today.date(15).format("YYYY-MM-DD"), title: "Client C - 3 PM", type: "followup" },
    ]);
    ...
    const daysArray = Array.from({ length: firstDayOfMonth + daysInMonth }, (_, index) =>
        index < firstDayOfMonth ? null : today.date(index - firstDayOfMonth + 1).format("YYYY-MM-DD")
    );
    ...
    const handleSaveAppointment = () => {
        ...
        setAppointments((prev) =>
            prev.some((app) => app.id === editingAppointment.id)
                ? prev.map((app) => (app.id === editingAppointment.id ? editingAppointment : app))
    ...
    const handleDragEnd = (event: any) => {
        ...
        setAppointments((prev) =>
            prev.map((app) => (app.id === Number(active.id) ? { ...app, date: over.id } : app))
        );
    };
    ...
    return (
        <div className="min-h-screen p-8 flex flex-col items-center bg-gray-100 dark:bg-gray-900 py-20">
            ...
            <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={daysArray.filter(Boolean) as string[]} strategy={verticalListSortingStrategy}>
                                ...
                                <SortableItem key={index} id={date || ""}>
                                    <motion.div
                                        className={`relative aspect-square flex flex-col items-center justify-center p-3 rounded-lg text-gray-700 dark:text-gray-300 transition-all
                                            ${date ? "cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-800" : "opacity-0 pointer-events-none"}
                                            ${selectedDate === date ? "border-2 border-blue-500 dark:border-blue-400" : ""}`}
                                        whileHover={{ scale: 1.05 }}
                                        onClick={() => setSelectedDate(date)}
                                    >
                                        {date && dayjs(date).date()}
                                        
                                        {appointments.some((app) => app.date === date) && (
                ...
                >
                    ...
                    <div className="mt-3">
                        {appointments
                            ...
                            .map((app) => (
                                <div key={app.id} className="p-3 flex justify-between items-center border-b">
                                    <div>
                                        <span className="text-gray-800 dark:text-gray-200">{app.title}</span>
                                        <span
                                            className={`ml-2 px-2 py-1 text-xs font-medium rounded-full ${appointmentTypes[app.type]}`}
                                        >
                                            {app.type.charAt(0).toUpperCase() + app.type.slice(1)}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <button onClick={() => { setEditingAppointment(app); setIsModalOpen(true); }}>
                        ...
                        {!appointments.some((app) => app.date === selectedDate) && (
